...
const MonthpickerInput = ({
  ...
  const renderEndElement = () => (
    <IconButton
      ...
      aria-haspopup="true"
      icon="calendar"
      id={`monthpicker-toggle-calendar-button-${input.name}`}
  ...
  const months = useMemo(
    () => Array.from(
      { length: 12 },
      (_, i) => new Intl.DateTimeFormat(intl.locale, { month: 'short' }).format(new Date(2000, i, 1))
    ), [intl.locale]
  );
  ...
  return (
    <div>
      <div ref={containerTextField}>
        <TextField
          aria-label={intl.formatMessage({ id: 'stripes-leipzig-components.monthpicker.yearMonthInput' })}
      ...
      >
        ...
        >
          <HasCommand
            ...
            scope={calendarRef}
          >
            {/* Popper component requires a 'div',
                which is why 'dialog' can not be used here and 'role' is set instead */}
            {/* eslint-disable-next-line */}
          <div
            ref={calendarRef}
            aria-label={intl.formatMessage({ id: 'stripes-leipzig-components.monthpicker.yearMonthSelection' })}
          ...
          >
            ...
            <fieldset className={css.calendarHeader}>
              <legend className="sr-only">
                {intl.formatMessage({ id: 'stripes-leipzig-components.monthpicker.yearSelection' })}
              </legend>
            ...
            <div
              aria-label={intl.formatMessage({ id: 'stripes-leipzig-components.monthpicker.monthSelection' })}
              className={css.calendarMonths}
              role="grid"
            >
              {months.map((monthLabel, i) => {
                const ariaLabel = i + 1 === lastValidDateRef.current.month
                  ? `${monthLabel} ${intl.formatMessage({ id: 'stripes-leipzig-components.monthpicker.selected' })}`
                  : monthLabel;
