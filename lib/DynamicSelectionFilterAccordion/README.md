...
The component ships type declarations in `DynamicSelectionFilterAccordion.d.ts`, so it can be used from TypeScript modules without further setup.
...
The value type of the filter (`string` by default) is inferred from `activeFilters` and `filterHandlers`; pass it explicitly for numeric filter values, e.g. `<DynamicSelectionFilterAccordion<number> ... />`.
